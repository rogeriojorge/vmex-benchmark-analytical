...
def main():
    ...
    full_map = np.zeros((ns, SOLVE_GRID, SOLVE_GRID), dtype=np.float64)
    map_rows = []
    ...
    for j, normalized_flux in enumerate(s_full):
        ...
        TH, PH, a, tangent = _surface_rates(case, label, SOLVE_GRID)
    ...
    run_id = datetime.now(timezone.utc).strftime("sheared-A-straight-field-%Y%m%dT%H%M%S.%fZ")
    run_id, out = reserve_run_directory(OUTPUT_PARENT, run_id)
    seed_path = out / "seed_ns17.npz"
    ...
    report = {
        "schema": 1,
        "evidence": "exact_surface_fieldline_lambda_projection",
