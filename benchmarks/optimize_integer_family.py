...
design).  An epsilon-constraint sweep on the other objective gives a small Pareto set.
"""
...
def optimize(args):
    ...
    results = []
    goal, capped = args.minimize, ("C_B" if args.minimize == "C_J" else "C_J")
    for kappa in args.caps:
        for start_index, start in enumerate(starts):
            ...
            cap = [{"type": "ineq", "fun": lambda t, k=kappa: k - float(value(jnp.asarray(t))[capped]),
                    "jac": lambda t: -np.asarray(grads(jnp.asarray(t))[capped])}]
            res = minimize(objective, start, jac=lambda t: np.asarray(grads(jnp.asarray(t))[goal]),
                           method="SLSQP", bounds=bounds, constraints=cons+cap,
                           options={"maxiter": 200, "ftol": 1e-12})
            results.append({"minimized": goal, "capped": capped, "cap": kappa, "start": start_index, "success": bool(res.success),
...
def main(argv=None):
    ...
    parser.add_argument("--minimize", choices=("C_J", "C_B"), default="C_J")
    parser.add_argument("--caps", type=float, nargs="+", default=(0.02, 0.04, 0.08, 0.16),
                        help="epsilon-constraint upper bounds on the other objective")
    ...
    for row in results:
        print(row["minimized"], row["cap"], row["start"], row["success"], ["%.4f" % v for v in row["theta"]],
