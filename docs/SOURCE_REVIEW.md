...
Preparation used the connected GitHub reader to inspect current VMEX changes, documentation, selected implementation sections and adjacent-library interfaces. Later local follow-through pinned VMEX, ran its baseline suite and GPU experiments, and expanded selected source/test reviews. No whole-tree semantic review or complete semantic audit is claimed here.
...
| `vmex/core/multigrid.py`, lines 259-310, 395-445 | Fixed-boundary ladder accepts explicit `ns_array`, `ftol_array`, `niter_array`, `tcon0`, initial state and nonconverged terminal-state policy; each stage passes `tcon0` into `prepare_runtime` | TCON0 changes the discrete force inside the solve, not just a post-hoc residual report. Reviewed the stage dispatch and ladder test paths; no fixed-boundary test directly certifies physical sensitivity to TCON0. |
...
| `tests/test_multigrid_ladder.py`, lines 1-260 | Structural executable reuse, warm start, multigrid convergence/golden parity, decreasing-grid policy | The suite tests ladder and convergence behavior but does not vary TCON0 against an independent physical-field oracle; S03b supplies that separate comparison. |
...
| `PlasmaControl/DESC` | At `4f48720beac3d4169e9165923d730445118bc2de`, targeted reads covered `Equilibrium.solve`, fixed-boundary constraints, `set_initial_guess`, `ForceBalance`, `FixThetaSFL`, `FourierRZToroidalSurface.from_values`, custom-grid dependency handling, B/J/grad-p implementations, and two related tests. One surface-fit test passed; native axisymmetric and integer-3D projections and DESC force-balance solves were scored at common held-out points. | A focused partial review and initial native comparison only. The DESC equilibrium test module could not be collected: it imports `Qic`, while the available PyPI package of the same name exports no `Qic`. The full DESC source/test review, independent transform measurement, other current closure, more remaps, derivatives, VMEC2000/VMEC++ and GVEC remain open. |
...
### DESC coordinate-comparison source review (partial)

The review ledger has partial entries for the following files at the pinned DESC revision. `Equilibrium.solve` defaults to the `force` objective and fixed-boundary/profile constraints; its `ForceBalance` objective uses radial and helical force residuals. The optional `FixThetaSFL` constraint is not part of that default. `set_initial_guess` accepts sampled R/Z/lambda on a supplied grid. `FourierRZToroidalSurface.from_values` fits sampled R/Z at caller-specified theta, assumes cylindrical phi is zeta, and documents angular sampling requirements. With `override_grid=True`, `Equilibrium.compute` evaluates dependencies that require surface or volume integration on suitable grids and transfers data back to custom points. DESC's B and J use its covariant/contravariant field and geometry derivatives; pressure gradient follows the profile derivatives and contravariant basis.

The inspected tests were `TestFourierRZToroidalSurface.test_surface_from_values` (passed) and `test_backward_compatible_load_and_resolve` (not collected because `tests/test_equilibrium.py` imports `Qic`, unavailable from the installed same-named PyPI package). This does not certify the whole test module or all solve paths. The exact source hashes, inspected ranges, tests and finding summaries are in `results/audit/review_ledger.json`.
