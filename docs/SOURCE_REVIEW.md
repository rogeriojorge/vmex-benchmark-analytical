...
| `PlasmaControl/DESC` | A clean default-branch checkout was fast-forwarded to `4f48720beac3d4169e9165923d730445118bc2de`; inspected `Equilibrium.solve` signature/default objective, `ForceBalance` setup, `FixThetaSFL` definition and `FourierRZToroidalSurface.from_values` angle input | Native comparison is planned in P3. This targeted interface read is not a semantic source/test review, environment validation, projection or solver run. Review the reached implementations and tests before attributing coordinate behavior to DESC. |
