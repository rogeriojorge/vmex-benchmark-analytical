...
| T3 sensitivities | NS17 dense solve certifies the linear response; NS65 frozen-path FD converges; branch inversion is stable. The 96-point radial m=1 candidate tracks the branch but produces a non-null physical B response of 0.184, so it is not a gauge. The matched NS65 TCON0=0 branch returned three states but their residuals exceed the 1e-11 certificate by 8.4k–12.1k; its B-FD is not a certified response. No derivative is certified. | Diagnose the TCON0=0 residual floor and gauge conditioning on saved states; only repeat the branch comparison after all roots meet the same residual certificate |
| T4 breadth | The exact-surface field-line lambda projection reduces matched 96-point B/J/force errors by factors 190/52/50. TCON0=1 converged in 208 iterations but worsened the projected physical score; TCON0=0 oscillated to the 10,000-iteration cap and scored worse than the TCON0=1 root. Both are unaccepted, unresolved NS17 diagnostics. | Keep both constraint settings as one-seed evidence only; resolve the physical scorer and investigate projection-to-root movement before broader recovery or any default recommendation |
...
### Publication and handoff checkpoint, 2026-09-25

**Published branch:** Commit `f2232e7e27158071dd6c1f893d8705684471be10` (`Continue VMEX response and sheared-A diagnostics`) contains the full staged continuation: 219 paths, including the preserved d5484d1 review handoff, the byte-identical archived plan, all new code/figures, and immutable numerical reports/arrays. It was pushed to `rogeriojorge/vmex-benchmark-analytical` on branch `t0-t1-d5484d1`. GitHub's commit API reports both author and committer as `rogeriojorge`; the exact owner noreply identity was used locally, and no co-author trailer is present. `origin/main` remains `d5484d1e7a15c9cf10599c2b0f05bdf69e22e861`. No PR was opened, no upstream branch was pushed, and no upstream change was merged.

**Handoff checks:** The supplied review package's nine checksums and the refreshed repository manifest's 62 checksums validate. `docs/history/plan_through_d5484d1.md` is byte-identical to `plan.md` at the reviewed base. The final staged privacy scan passed; the largest new file was 1.76 MiB. The portable suite, current-environment suite, 14-case analytic reference verification, compileall, JSON parse and README figure-link checks are recorded above. The only staged whitespace findings were ten intentional Markdown hard breaks in the supplied handoff/history copies, retained byte-for-byte.

**Exact next action:** Start with a small saved-data NS65 analysis of the active m=1 state-FD minus residual-tangent combination. For any proposed alignment, reconstruct Cartesian B before and after and require a measured invariant-field bound; do not infer a gauge from mode concentration and do not change the global TCON0 default. In parallel, derive the sheared-A VMEX lambda map by integrating exact field-line flow, check straightness and edge closure, Fourier-project the nonzero lambda state, and compare residual and the same 96-point physical scores to the saved lambda-zero projection before another recovery. Keep the historical VMEX source at `b5f5267efc0795c4a49a224e321e9b370975c14c` distinct from any pinned current snapshot. Record each bounded run's input/state hashes, residuals, physical scores, timings and peak RSS; update the task table and append the next log entry before proceeding to wider T4 cases.


### Continuation work block, 2026-09-25: exact sheared-A field-line lambda projection

**Question/task:** Is the missing VMEX lambda coordinate the main reason that a geometry-only projection represents the sheared-A magnetic field and current poorly, and does the exact field-line map fit the NS17 basis accurately enough to justify one bounded recovery attempt?

**Source/input/method:** Historical VMEX `b5f5267efc0795c4a49a224e321e9b370975c14c`, version `0.11.0`; processed current-prescribed sheared-A input SHA-256 `def5ce4ef00329b7c7c7c69c1c60821d2e6d1c8d7db5c3694c8ed17d6de78897`; NS17, MPOL=13, NTOR=12, NFP=2. Solved the periodic surface equation `D_phi(lambda) + a D_theta(lambda) = iota - a`, where `a=(B dot grad(theta))/(B dot grad(phi))`, with a zero-mean gauge. Cartesian x/y are antiperiodic over one field period, so their direct spectral derivative was rejected after its tangent-field residual reached 0.49. The accepted implementation differentiates periodic cylindrical R/Z and constructs the physical cylindrical tangent vectors. The least-squares solve used a 48x48 angular grid, at most 5,000 LSMR iterations and a 64x64 independent check grid. VMEX state conversion follows its WOUT lambda scaling: physical lambda times `phipf/lamscale` and the native physical-to-internal mode factor.

**Map verification:** The field tangent reconstructed from the two coordinate tangents has relative error `1.83e-12` at the representative s=0.5, 48x48 surface. On that surface, the relative straightness residual decreases from `4.43e-6` at 24x24 to `5.01e-8` at 32x32 and `8.46e-10` at 48x48; lambda RMS changes by less than `1.2e-10` between 24 and 32. On independent 64x64 grids at s=0.25, 0.65 and 0.94, relative straightness residuals are `7.07e-10`, `3.34e-9` and `5.63e-9`. The retained MPOL=13/NTOR=12 Fourier lambda fit errors on those surfaces are at most `1.45e-6` relative; over the 16 solved radial surfaces the largest 48-grid fit error is `1.84e-6`. The total cosine-to-sine lambda coefficient norm ratio is `2.04e-13`, consistent with the symmetric input basis. The map report records all 17 radial rows including the axis coefficient closure.

**Matched physical projection result:** The geometry coefficients are bitwise identical to the earlier lambda-zero seed; its recorded processed-edge coefficient mismatch remains at most `5.13e-17 m`. The input hash, historical VMEX pin, 96 reference Cartesian points, weights and native flux labels are identical to the lambda-zero projection; reference sample NPZ SHA-256 is `45ee4217c6c01efd41cd211d12059bf76dbf86830567956806164b83085af5c2`. Thus only the lambda state changed. On these same 96 points, B/J relative L2 errors improve from `0.134055 / 0.265278` to `7.0655e-4 / 5.1312e-3`; force RMS divided by pressure-gradient RMS falls from `1.20916` to `2.4406e-2` (about 190x, 52x and 50x reductions). Grad-p error is unchanged at `3.5814e-4`, as expected for the same geometry and flux labels. Maximum native/reference flux-label difference remains `2.6787e-5`. The map-projected state is still not an equilibrium: VMEX invariant residuals are FSQR/FSQZ/FSQL `0.163998 / 0.088742 / 2.2150e-7`, and preconditioned R/Z/lambda block norms are `7.956e-3 / 5.835e-3 / 4.060e-4`. The physical measurement is still a 96-point diagnostic, not resolved-volume evidence; recovery, root certification and acceptance are all false.

**Command/resources/artifacts:** `PYTHONPATH=benchmarks:<pinned VMEX checkout> python3 benchmarks/project_sheared_straight_field.py`; 56.49 s elapsed, 2,624.55 MiB peak host RSS; CPU execution, no device-memory claim. Script SHA-256 `ad5e52fa82cb34693b75829111fc3e8bc7038068e1828c1efc00a216eaeffbaf`. Report `results/projection/sheared_A/sheared-A-straight-field-20260925T004606.885805Z/straight_field_projection.json` SHA-256 `334309a6f50c5a4d4932ba284ea9e49e2a3871dd8980b78045bf2ce5efd964dd`; seed SHA-256 `d198877581b9191bb03736b66d5de7bf01603dfdf26974814724031039635957`; lambda map SHA-256 `ddf1b40132b4b5555a19e570026c745cb2aae47f3c7ea5f58b3cc3f1437a4fad`; native observations SHA-256 `0ff555e4f064f4eda85547cadec82601a7d9480ab923c6207b080741c93ad8c0`. The report also contains the byte-identical reference-grid hash and full environment record.

A read-only plotting script generated and visually checked `figures/vmex_sheared_a_lambda_projection.png`, SHA-256 `72d4807c563bb464d86e48eb5d00c5f80ba6f058feb58702b6a76836756d0d54`. Its manifest `results/projection/sheared_A/lambda_projection_figure_manifest.json` checks both report hashes, identical VMEX/source/input/sample grid, and identical R/Z state arrays. README reports the projection improvement without calling it a solve.

**Branch/PR state:** Continuation branch `t0-t1-d5484d1` still points to published commit `b1dd8a97be666247c435de588e20d612143c7ca4` before this block is staged. No PR or upstream write; historical and current VMEX pins remain separate. No default constraint or solver code was changed.

**Exact next action:** Run one bounded historical NS17 sheared-A current-prescribed solve from the saved nonzero-lambda seed with `FTOL=1e-10`, `NITER=10000`, default TCON0 and terminal-state preservation. Compare its actual root residual, convergence flag, native scores, flux labels, elapsed time and peak RSS against the prior cold and lambda-zero-seed runs. Keep the volume measurement unresolved and do not expand the iteration cap if it stops. Separately, continue the pending NS65 active-m=1 alignment test against physical B; no gauge claim or global constraint change is authorized by the present projection result.

### Continuation work block, 2026-09-25: bounded recovery from the field-line lambda seed

**Question/task:** Does a VMEX current-prescribed NS17 solve starting from the exact-surface straight-field lambda projection preserve its strong analytical field score while reducing the VMEX residual?

**Run/source/input:** Historical VMEX `b5f5267efc0795c4a49a224e321e9b370975c14c`; input SHA-256 `def5ce4ef00329b7c7c7c69c1c60821d2e6d1c8d7db5c3694c8ed17d6de78897`; seed SHA-256 `d198877581b9191bb03736b66d5de7bf01603dfdf26974814724031039635957`; parent projection run `sheared-A-straight-field-20260925T004606.885805Z`. Command: `PYTHONPATH=benchmarks:<pinned VMEX checkout> BENCH_FTOL=1e-10 BENCH_RUN_ID=sheared-a-ns17-straight-field-terminal-20260925 BENCH_PARENT_RUN_ID=sheared-A-straight-field-20260925T004606.885805Z python3 benchmarks/run_vmex.py inputs/input.sheared_A_current 17 10000 results/projection/sheared_A/sheared-A-straight-field-20260925T004606.885805Z/seed_ns17.npz --keep-terminal`. Effective NS=17, FTOL=`1e-10`, NITER=10,000, default TCON0=1, current-prescribed closure. The nonlinear solver stopped at 208 iterations with IER=11 and FSQR/FSQZ/FSQL `9.8106e-11 / 7.0456e-11 / 3.1144e-11`. This is a solver-converged discrete root, but it is not an accepted physical result: the saved run explicitly has `accepted=false`, `root_certified=false`, and `measurement_resolved=false`.

**Matched physical result:** The runner's 96-point `point_cloud.npz` is byte-identical to the exact projection's reference sample array, SHA-256 `45ee4217c6c01efd41cd211d12059bf76dbf86830567956806164b83085af5c2`. On those fixed points, the converged root has B/J/grad-p relative L2 errors `1.5090e-3 / 3.1412e-2 / 1.4958e-3` and force divided by pressure-gradient RMS `0.14224`. The starting mapped projection scored `7.0655e-4 / 5.1312e-3 / 3.5814e-4 / 0.024406`, so the root is about 2.14x, 6.12x, 4.18x and 5.83x worse on these physical metrics. It is nevertheless better than the previous capped cold/lambda-zero-seed terminal states. Its maximum native/reference flux-label error is `7.2538e-4`, versus `2.6786e-5` for the mapped seed. All three runner thresholds (B `1e-5`, J `1e-3`, force scale `1e-3`) fail; this 96-point measurement remains unresolved. The result demonstrates on this run that reducing the discrete force residual does not guarantee reducing analytical physical error; it does not identify the constraint as the cause.

**Timing/artifacts:** Solve time `2.771 s`; native sample/score time `29.523 s`; peak host RSS `2687.08 MiB` on CPU; device memory unavailable. Forward report `results/vmex/runs/sheared-a-ns17-straight-field-terminal-20260925/forward.json` SHA-256 `c778a79d641fada715efcc70188eed260b00f3c9ec58f0d6539338a5b92f70e1`; score JSON SHA-256 `df1037bc12d239f5dce58d4cc36f3a456ee33a6c36ab0bf540a581cf796d22c9`; WOUT SHA-256 `fbd0eb85fca3ede0b7c64162887a900c43b51f3d598246eff44f1e9b98e8238e`; saved input copy hash equals the source input hash. The new matched figure `figures/vmex_sheared_a_lambda_recovery.png` was visually inspected; SHA-256 `892d62c11adee6c5f5ac67e45d0112b2b0de3659df1a7f462f86eb70d35f5fa4`; its manifest binds the projection/root reports, identical point cloud and initial seed.

**Validation/branch:** After the field-line projection block, `python3 -m pytest -q` passed 55 tests in 15.59 s; `python3 benchmarks/verify_reference.py` passed all 14 cases; compileall passed; 206 result JSON files parsed; and `git diff --check` passed. The new plotting script was executed successfully and its output visually reviewed. The continuation branch still has published tip `b1dd8a97be666247c435de588e20d612143c7ca4` before the current block's uncommitted files. No PR, upstream write, default constraint change or solver source change.

**Exact next action:** Use the same source, input, lambda seed, NS17, FTOL=`1e-10` and NITER=10,000 for one paired diagnostic with explicit per-run `TCON0=0`; preserve its terminal state and do not change the global default. Compare its actual VMEX residual, convergence, B/J/grad-p/force scores, flux-label drift and memory with this TCON0=1 result. This determines whether the coordinate penalty is implicated in the projection-to-root physical degradation for this sheared case. Keep the axisymmetric m=1 response/gauge hypothesis separate, and require actual physical-field invariance before making a gauge claim.

### Continuation work block, 2026-09-25: matched TCON0 control from the field-line seed

**Question/task:** For the same NS17 exact-geometry/field-line seed and current-prescribed input, does setting the constraint weight to zero yield a more physically accurate root or a better-behaved solve than the deck-default TCON0=1 run?

**Run/source/input/seed:** Historical VMEX `b5f5267efc0795c4a49a224e321e9b370975c14c`; input SHA-256 `def5ce4ef00329b7c7c7c69c1c60821d2e6d1c8d7db5c3694c8ed17d6de78897`; seed SHA-256 `d198877581b9191bb03736b66d5de7bf01603dfdf26974814724031039635957`; same NS17 current-prescribed setup and parent projection as the TCON0=1 run. Command: `PYTHONPATH=benchmarks:<pinned VMEX checkout> BENCH_FTOL=1e-10 BENCH_TCON0=0 BENCH_RUN_ID=sheared-a-ns17-straight-field-tcon0-zero-terminal-20260925 BENCH_PARENT_RUN_ID=sheared-A-straight-field-20260925T004606.885805Z python3 benchmarks/run_vmex.py inputs/input.sheared_A_current 17 10000 results/projection/sheared_A/sheared-A-straight-field-20260925T004606.885805Z/seed_ns17.npz --keep-terminal`. Effective TCON0=`0`, FTOL=`1e-10`, NITER=10,000. This was a per-run override; the input and global/default TCON0 were not changed.

**Outcome:** The first printed residual was FSQR/FSQZ/FSQL `3.13e-6 / 3.06e-6 / 2.22e-7`; the log then alternated between low `~1e-9` residuals and excursions above `1e-7`, through all 10,000 iterations. The run capped with IER=2 and final FSQR/FSQZ/FSQL `6.8443e-7 / 7.1405e-7 / 4.4510e-7`. It did not converge. On the byte-identical 96-point cloud (SHA-256 `45ee4217c6c01efd41cd211d12059bf76dbf86830567956806164b83085af5c2`), its B/J/grad-p relative L2 errors are `6.4375e-3 / 4.6121e-2 / 8.0214e-2`, force/pressure-gradient RMS is `0.17534`, and max native/reference flux-label error is `0.05255`. All score thresholds fail; root certification and spatial measurement resolution remain false.

The paired TCON0=1 run from the same saved seed did converge in 208 iterations at FSQR/FSQZ/FSQL `9.8106e-11 / 7.0456e-11 / 3.1144e-11`, and its matched physical scores `1.5090e-3 / 3.1412e-2 / 1.4958e-3 / 0.14224` are better than the TCON0=0 capped terminal state. Both solver outputs are worse than the exact field-line projection on the same points. The experiment therefore does not show that removing the coordinate penalty helps this sheared-A initialization; it does not establish that the default is optimal in general, nor explain why the solver leaves the more accurate projected state.

**Timing/artifacts:** Solve `24.342 s`; sample and score `30.133 s`; peak host RSS `2697.38 MiB` CPU; device-memory use unavailable. Forward report `results/vmex/runs/sheared-a-ns17-straight-field-tcon0-zero-terminal-20260925/forward.json` SHA-256 `9b90709cc199d8e41451d4a38addf08f742f911f3eed481aa2a6bc89de91db5f`; score JSON SHA-256 `e3b0a06d29c34b5c4881bed27db9b91738342b83f0f75d523328f3995d4475fe`; WOUT SHA-256 `c699f3778c13b34c109b805deffc0fd59591fd3062b8a487c14704cab6eb22b0`; observations SHA-256 `cf3dfa54f045cad39c4c38f01cdbf2a3a7bdc4961c8073f048663e5647f87a70`. The updated matched figure `figures/vmex_sheared_a_lambda_recovery.png` SHA-256 `a6111904af01a6002de481ee8dd1864ef45376797b9098f9c4e562eb68a35ae6`; it was visually checked and the manifest binds four same-grid states and their reports.

**Branch/PR state:** Continuation branch remains at previously published `b1dd8a97be666247c435de588e20d612143c7ca4` before this block's work is staged. No PR, upstream write, solver change or global default change. README and plan now distinguish the exact projection, solver-converged TCON0=1 state and capped TCON0=0 state.

**Exact next action:** Resume the pending NS65 axisymmetric active-m=1 state-FD versus residual-tangent analysis. Test any proposed coefficient alignment by reconstructing physical Cartesian B before and after on the saved fixed points and a held-out set; report the candidate transformation's actual B change, not only coefficient correlation. Keep the sheared-A TCON0 result as a single NS17 paired diagnostic. Before committing, run the current full tests and reference verifier, inspect the complete staged diff and privacy scan, verify owner identity, and push only the continuation branch.

### Continuation work block, 2026-09-25: physical invariance check for the NS65 m=1 relabel candidate

**Question/task:** Does the radial `m=1` coefficient alignment that improves agreement with the independently reconverged branch preserve the exact family's null Eulerian B response, or does it merely track a physically displaced branch?

**Source/input/method:** Historical VMEX `b5f5267efc0795c4a49a224e321e9b370975c14c`; input SHA-256 `fc3e7591846aa18bb2365a2db731e865baa5d569ea5bfe4c9392bd71b7fc6852`; NS65, exact-null `delta` direction, centered step `h=1e-5`; 96 fixed Cartesian points on six normalized-flux surfaces at 16 poloidal angles and fixed toroidal angle. Used the saved base and independently anchored plus/minus roots, fit the `m=1` geometry gap to `eta(s) d(R_1,Z_1)/ds`, differentiated the native physical field along the candidate, and then formed a centered field finite difference from `base_state ± h*candidate_tangent` with the same saved signed input pair. Command: `PYTHONPATH=benchmarks:<pinned VMEX checkout> python3 benchmarks/validate_axisymmetric_relabel_field.py`. The script hash for this run is `43d9d7b457dd9c873342906e91f88a3584031b2477eb928a388498710829e151`.

**Result:** Base/plus/minus and candidate plus/minus coordinate inversions were all valid; the base native-flux-label error is at most `1.6426e-5`, and both saved branch roots have anchored residuals at most `8.0628e-14`. The measured branch B-FD norm is `0.183959` in the fixed field-response scale, even though the exact analytical `delta` derivative at fixed Cartesian points is zero. The residual-tangent B JVP has norm `0.186247` and differs from branch B-FD by `0.028771`. The radial-relabel candidate JVP has norm `0.183978` and differs by `0.0018399`; the candidate centered field FD has norm `0.183971`, differs from branch B-FD by `0.0018905`, and differs from its own JVP by `1.8632e-4`. The measured-branch-state JVP differs from branch B-FD by `1.7767e-4`. Thus radial relabeling tracks this branch response closely but changes physical B by about `0.184`; it is not a null/gauge transformation. The earlier coefficient fit's 2.85% geometry residual and only small reduction of the full-state gap also remain. No derivative or coordinate change is accepted.

**Artifacts/resources:** Report `results/audit/axisym_radial_relabel/axisym-relabel-field-20260925T010115.952375Z/radial_relabel_field_test.json` SHA-256 `953727846442621e9c780f296b2c552e50729af0aea1ae4bc74a8d72f393c453`; saved arrays SHA-256 `c9b9856f48a90b7a7e2bc6e531b3ec19105052e536c3b837f14fa69d2292df11`; elapsed `18.392 s`; peak host RSS `1808.08 MiB` CPU. The figure `figures/vmex_axisym_m1_branch_alignment.png` was visually inspected; its SHA-256 is `fb2bfedd5bce6b4579105860bdee9cef9dc823ba6bc2d92a0445335656a00ba0`; the manifest binds the report and array hashes.

**Branch/PR state:** Continuation branch is still at published `b1dd8a97be666247c435de588e20d612143c7ca4` before this work is staged. No PR or upstream write. No constraint default or solver source was changed.

**Exact next action:** Run a matched NS65 exact-null `delta` branch at TCON0=0: reconstruct the same base and `h=1e-5` plus/minus input maps, solve/anchor all three using the same historical source, sample the same 96 Cartesian points, and compare branch B-FD and true root residuals against the saved TCON0=1 states. This is a per-experiment override only. If the zero-weight roots cap or their physical response remains nonzero, preserve the failure and do not infer a general default recommendation.


### Continuation work block, 2026-09-25: matched NS65 TCON0=0 branch has no root certificate

**Question/task:** Does the exact-null NS65 `delta` branch response change when the coordinate penalty is set to zero, compared with the same base and centered `h=1e-5` perturbations at default `TCON0`? The branch was sampled only after solving the same three parameter decks at `TCON0=0`; no global default or VMEX source was changed.

**Source/input/configuration:** Historical VMEX `b5f5267efc0795c4a49a224e321e9b370975c14c`, tracked tree clean; NS65, `FTOL=1e-12`, `NITER=10000`, `refine_tol=1e-11`, and `TCON0=0` as a run-only override. Base input SHA-256 `fc3e7591846aa18bb2365a2db731e865baa5d569ea5bfe4c9392bd71b7fc6852`; generated base/plus/minus deck SHA-256 values `7a75525e3efcd626f96811bec3c243b3dc8911f0832bcb2249cc46c9c4142deb`, `3ac64e9ec1df13abdfefd97bcf9a2520443c9a5a30a60ec5b7edab672be0d5b0`, and `19a3622f7239616aa6da0ecea68e1733d9e8a5252666952a3d41a3dcc8b14cf2`. Sampled the same 96 fixed Cartesian points on six flux surfaces as the default branch. Effective settings were confirmed in the immutable report.

**Command and result:** `PYTHONPATH=benchmarks:<pinned VMEX checkout> python benchmarks/probe_axisymmetric_tcon0_branch.py`. All three host solves returned after 1,091 iterations. The root-refinement lane recorded 7 steps and 6,151 Krylov iterations per receipt, but made no state change. Post-refinement residuals were `1.188382947e-7`, `8.423407042e-8`, and `1.210137839e-7`, exceeding the `1e-11` certificate tolerance by factors `11,884`, `8,423`, and `12,101`. Thus these are solver-returned unresolved states, not certified roots. All 96 Cartesian field inversions were valid. The TCON0=0 branch B-FD norm is `0.3980116`, compared with `0.1839587` for the saved default branch, and the difference norm is `0.4903191`. Because the zero-strength roots are not certified, none of these branch differences estimates a valid equilibrium sensitivity. Do not infer that TCON0=0 worsens the physical branch or recommend a constraint default from this result.

**Status amendment and source correction:** The probe's original row label `anchored_diagnostic_root` overstated what the residual established. The immutable raw report was not edited. A separate amendment classifies all three as `residual_unresolved`, links the original report and recorded script hashes, and records the `1e-11` certificate rule. The corrected checked-in probe distinguishes returned states from certified roots; it can still sample a returned branch while recording `all_roots_certified=false`. The current probe source hash `c133b0d4840e698f11785aff9974461df8204bdff3319ede23addd336b5a8d0f` does **not** match the script hash `f8f8fd6f521ee791590f71c6b357f96f27986c829edc19122daff4b4560bb0b2` recorded by the immutable run. The amendment records this mismatch; the exact run-time script bytes are not included in this checkpoint, so reproducing the precise historical command implementation remains a provenance gap and requires a fresh run of the corrected source after root-floor diagnosis. Original probe report SHA-256 `4ef56762a6485a3cddf99d2c0a7bec4395b0573075769d2a134ac40462f3bdb4`; arrays SHA-256 `c25dd1e08aa732a94dc2d4b39508f0dfa41bce301606c935867e87d675a3c66e`; recorded original run-script SHA-256 `f8f8fd6f521ee791590f71c6b357f96f27986c829edc19122daff4b4560bb0b2`. Amendment SHA-256 `07bf05d4472d19e8628ae2d4bf8b7eb9343ad808efa0db4d7c183af2f00f0e76`; amendment script SHA-256 `e3c1209fac39ad6f884dc9e6c7ec1ceb41d9f713ae7712dfb090775b54263177`.

**Resources/code review:** Elapsed `42.9203 s`; host peak RSS `2110.39 MiB`; CPU. Reviewed the benchmark's `_root_and_anchor`/fixed-point residual path and the pinned VMEX `implicit._refined_state` logic. The refinement attempts a block solve then GCROT fallback, keeps only a lower-residual state, and returns the original state if no step improves it. This explains the zero anchor shift recorded here but does not diagnose the underlying zero-strength conditioning. The updated root-status logic was syntax-checked; the full suite and reference verifier remain to be rerun before publication. No VMEX solver file was modified.

**Artifacts:** `results/audit/axisym_branch_tcon0/axisym-tcon0-branch-20260925T010824.995851Z/tcon0_branch_probe.json`, `branch_tcon0_ns65.npz`, three immutable root/deck/state receipts, and `root_status_amendment.json`. README now states the result as uncertified and links the probe plus amendment. The axisymmetric branch figure is unchanged because this result does not qualify as a solved response comparison.

**Branch/PR state:** Continuation branch `t0-t1-d5484d1` is based on published `b1dd8a97be666247c435de588e20d612143c7ca4`; this work is currently uncommitted and unpublished. No PR, upstream write, solver change, or global-default change.

**Exact next action:** Use the saved NS65 TCON0=0 base input and state for one bounded root-floor diagnostic. Record raw and preconditioned residual blocks, the accepted/rejected Newton correction sequence, and a small gauge/rank-conditioning probe. Do not rerun the plus/minus branches until the base residual can be reduced below `1e-11` and the saved TCON0=1 base/branch meet the same certificate. Then decide whether the matched TCON0 comparison is identifiable; retain all current field differences as diagnostic only.


### Validation checkpoint, 2026-09-25

**Commands/results:** `python -m py_compile` passed for all eight changed/new benchmark scripts. `pytest -q` passed all 55 tests in 15.90 s; after the final root-status correction, the same compile check and suite passed again (55 tests, 14.72 s). `PYTHONPATH=benchmarks python benchmarks/verify_reference.py` passed all 14 exact reference cases; maximum reported force identity residual was `1.65e-15` and maximum finite-difference residual was `2.01e-9`. `git diff --check` passed for both the working and staged diffs. The three newly generated figures were inspected at full presentation scale; labels and annotations are legible, and each manifest binds the source report and array hashes.

**Handoff integrity:** `HANDOFF_SHA256SUMS` verified every listed file except the intentionally updated active `README.md` and `plan.md`; their previous bytes match the manifest at parent `b1dd8a97be666247c435de588e20d612143c7ca4`. The original review-package checksum list remains preserved under `docs/handoff/review-d5484d1/`. Refresh the two active-document checksums before the publication commit, then rerun the checksum verification. No raw measurements or archived plans were rewritten.
...
### Main-branch consolidation checkpoint, 2026-09-24

**Repository consolidation:** Starting from reviewed `main` commit `d5484d1e7a15c9cf10599c2b0f05bdf69e22e861`, fast-forwarded local `main` to continuation commit `438b48a559eed65930d517b7c644f05c025047cd`. The four intervening commits (`f2232e7`, `b1dd8a9`, `58c194a`, and `438b48a`) and their complete tracked contents are now on `main`: source and test changes, the preserved historical plans and review handoff, the active plan/logbook, README figures, scripts, raw numerical reports, arrays, inputs, manifests, and amendments. No source solver or global constraint default was changed. The prior review handoff and all later evidence remain in Git history.

**Scientific status:** This is a repository/branch consolidation only; it does not promote diagnostic or unresolved experiments. Keep the statuses in the task table above. In particular, the NS65 TCON0=0 states remain uncertified, the NS65 branch derivative remains uncertified, the NS17 sheared-A outcomes remain unaccepted diagnostics, and no global TCON0 recommendation is supported. The exact next scientific action remains the bounded saved-base NS65 TCON0=0 root-floor diagnostic: record raw and preconditioned residual blocks, accepted/rejected correction sequence, and a small gauge/rank-conditioning probe. Do not repeat plus/minus branches unless the base and matched TCON0=1 roots satisfy the stated `<=1e-11` residual certificate.

**Validation and identity:** The continuation block's recorded validation is unchanged: 55 tests passed, all 14 reference cases passed, changed benchmark scripts compiled, and saved figures were inspected. These were run before this content-only fast-forward and are not claimed as new runs. GitHub authentication was verified as `rogeriojorge`; local author and committer are configured as `rogeriojorge <6816712+rogeriojorge@users.noreply.github.com>`. The public repository is `rogeriojorge/vmex-benchmark-analytical`, with `main` as its default branch. No PR or upstream write was created.

**Branch state and exact next repository action:** At this checkpoint, local `main` contains `438b48a559eed65930d517b7c644f05c025047cd` and is four commits ahead of `origin/main`. Push this verified `main` tip, confirm the remote matches it, and only then delete the fully contained continuation branch. The older handoff branch was already absent from the remote when refs were fetched and pruned. Finish with one local branch (`main`), one remote branch (`origin/main`), a clean worktree, and a valid `HANDOFF_SHA256SUMS`; record the resulting branch state in a final handoff receipt. No experiment is certified by this publication step.


### Final handoff receipt: single-main repository state

Pushed the consolidation to `origin/main`; the pre-cleanup main checkpoint `1e21dd50f6039b9b9eb1806a296e23b88bf0b42c` was confirmed identical locally and remotely. Deleted the fully contained `t0-t1-d5484d1` remote branch and both local continuation/review branch refs. The older handoff branch was already absent remotely at fetch/prune. The resulting repository has one local branch, `main`, and one remote branch, `origin/main`. The complete continuation history and all tracked code, test evidence, reports, arrays, inputs, figures, archived plans, and handoff documents are reachable from `main`; no commits were squashed or discarded. The commit containing this receipt will be pushed to that same sole branch.

The earlier validation results remain the latest test evidence; this receipt changes only handoff documentation and checksums. The checked-in README retains the polished saved-data figures and measured results, and `plan.md` retains the complete chronological logbook, unresolved findings, source pins, failure notes, and exact next actions. The checksum manifest is refreshed and must validate before handoff. Repository branch consolidation does not certify unresolved experiments or change the scientific task statuses.

**Next action for the continuing study:** Run the bounded saved-base NS65 TCON0=0 root-floor diagnostic described above. Preserve the failed-root classification, quantify raw/preconditioned residual blocks and actual correction attempts, and probe gauge/rank conditioning. Repeat no perturbed branches until the root certificate is met for the base and matched default-constraint case. Keep all new measurement reports, source identities, resource data, task-table updates, and logbook records on `main`; do not create another long-lived branch unless a specific experiment requires isolation.
