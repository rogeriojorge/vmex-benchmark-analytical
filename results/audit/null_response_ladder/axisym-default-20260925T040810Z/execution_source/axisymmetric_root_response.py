...
from evidence import capture_execution_source, reserve_run_directory, sha256_file, source_metadata, write_json
...
def _input_for_parameters(base_input, parameters):
    ...
    def pad(values, size=21):
        if len(values) > size:
            raise ValueError(f"fitted profile needs {len(values)} coefficients; interface supports {size}")
        return np.pad(np.asarray(values, dtype=float), (0, size-len(values)))
...
def main(argv=None):
    ...
    capture_execution_source(output, [Path(__file__), here/"refinement_observer.py", here/"measurement.py",
                                      here/"evidence.py", here/"build_inputs.py", here/"analytic.py"],
