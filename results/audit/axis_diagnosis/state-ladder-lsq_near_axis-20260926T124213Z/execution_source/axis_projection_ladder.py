...
def main(argv=None):
    ...
    parser.add_argument("--state-npz", type=Path, action="append", default=[],
                        help="evaluate these saved states (NS inferred) instead of fresh projections")
    parser.add_argument("--zero-lambda-rows", type=int, default=None,
                        help="diagnostic: zero L_sin/L_cos in radial rows < N of --state-npz states")
    parser.add_argument("--axis-row", choices=("pinned_copy", "linear_extrapolation", "lsq_near_axis"), default="pinned_copy",
    ...
    parser_fit_rows, parser_replace_rows = 8, 4  # lsq_near_axis: fit rows 1..8, replace rows 0..3
    if args.axis_row in ("linear_extrapolation", "lsq_near_axis"):
        ...
        def patched(coefficients, modes, spline=True):
            ...
            if args.axis_row == "lsq_near_axis":
                # Diagnostic: least-squares quadratic in s of the smooth regular
                # coefficient over rows 1..8, used for rows 0..3 (m > 0 only).
                rows = jnp.arange(1, parser_fit_rows+1)
                V = jnp.stack([s_mesh[rows]**k for k in range(3)], -1)
                coef, *_ = jnp.linalg.lstsq(V, regular[rows], rcond=None)
                Vr = jnp.stack([s_mesh[:parser_replace_rows]**k for k in range(3)], -1)
                fit = Vr @ coef
                regular = regular.at[:parser_replace_rows].set(
                    jnp.where(powers[None, :] > 0, fit, regular[:parser_replace_rows]))
            else:
    ...
    run_id, out = reserve_run_directory(
        args.output_parent, f"{'projection' if not args.state_npz else 'state'}-ladder-{args.axis_row}-{datetime.now(timezone.utc):%Y%m%dT%H%M%SZ}")
    ...
    jobs = ([(None, ns) for ns in args.ns] if not args.state_npz
            else [(path, None) for path in args.state_npz])
    for path, ns in jobs:
        if path is None:
            ...
            data = np.load(path)
            arrays = {n: np.asarray(data[n]) for n in observer.FIELDS}
            ns = arrays["R_cos"].shape[0]
            if args.zero_lambda_rows is not None:
                for n in ("L_cos", "L_sin"):
                    arrays[n][:args.zero_lambda_rows] = 0.0
            inp = replace(base, ns_array=np.asarray([ns]))
            state = SpectralState(**{n: jnp.asarray(v) for n, v in arrays.items()})
        ...
        row = {"ns": ns, "state_sha256": observer.tree_hash(state), "by_s": {},
               "source": "projection" if path is None else str(path),
               "zero_lambda_rows": None if path is None else args.zero_lambda_rows}
        for s in args.axis_s:
            ...
            gradB = np.asarray(_cartesian_derivative(spectra, 1, coords, valid)).reshape(-1, 3, 3)
            ...
            for tag, T in (("ij", gradB), ("ji", np.swapaxes(gradB, -1, -2))):
