...
def main(argv=None):
    ...
    parser.add_argument("--mpol", type=int, help="override the deck MPOL (resolution study)")
    parser.add_argument("--ntor", type=int, help="override the deck NTOR (resolution study)")
    ...
    if args.mpol is not None or args.ntor is not None:
        ...
        def _pad(array):
            # (2 ntor + 1, mpol) boundary table, n centred.  Zero-padding is
            # enough here: the projected edge row is fitted from the exact
            # surface, not read from the deck (a solve would need a refit).
            array = np.asarray(array)
            out = np.zeros((2*ntor+1, mpol))
            dn = ntor-(array.shape[0]-1)//2
            out[dn:dn+array.shape[0], :array.shape[1]] = array
            return out

        inp = _replace(inp, mpol=mpol, ntor=ntor,
                       **{k: _pad(getattr(inp, k)) for k in ("rbc", "rbs", "zbc", "zbs")})
