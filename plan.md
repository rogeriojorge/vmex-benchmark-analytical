...
**Upstream PR opened, 2026-09-25:** [uwplasma/vmex#452](https://github.com/uwplasma/vmex/pull/452), branch `fix/extender-axis-row-extrapolation`, commit `afd1392` (author and committer rogeriojorge), on main `926892ab`. Preflight: changed-line coverage 100%. One local failure (`test_virtual_casing_physics::test_the_derivative_check_stays_quiet_many_surface_sizes_away`, NumPy 2.5.1 overflow warning on macOS) reproduces on unmodified main and passes on Linux; it is unrelated to the change. Office regression run: optimize, problem, near-surface and virtual-casing files 75 passed, 21 skipped. Not merged; the benchmark keeps the historical pin. **Next:** after CI, rescore the NS129/NS257 zero-TCON0 states and the NS129 axisymmetric responses with the patched evaluator, as a separately labelled evaluator variant.

### C3/C5 addendum, 2026-09-25 (office)

**Integer Newton control:** `observe_root_refinement.py --no-host-solve --passes 4 --saved-state results/projection/integer_3d_vmex_ns129/seed.npz --ns 129` (run `integer3d-ns129-seed-newton-control-*`): 6.67e-2 -> 7.21e-3 -> 9.74e-4, then no progress; not certified, even though the host solver converges from this seed in 39-50 iterations. A pure Newton polish from a raw projection has a small basin in both families, so this control does **not** discriminate sheared-A from integer. The sheared-A difficulty lies in the host descent path, not the refinement. **Next for C3:** a host-solve continuation (NS17 root -> NS33 -> NS65 multigrid from the full-flux seed) and one matched TCON0=0 control, recording the flux-label drift per stage.

**Solov'ev LASYM (C5):** `BENCH_FTOL=1e-12 run_vmex.py inputs/input.solovev_{symmetric,asymmetric}_current {33,65} 30000` (runs `solovev-*-ns{33,65}-20260925`). All converged (IER 11). Symmetric, native route, B/J/grad-p/force: NS33 1.24e-5 / 1.45e-3 / 2.55e-5 / 1.65e-2; NS65 4.86e-6 / 1.81e-4 / 2.74e-5 / 2.03e-3. Genuinely up-down-asymmetric (LASYM=T), fitted-state route: NS33 1.53e-4 / 5.75e-3 / 1.87e-4 / 6.47e-2; NS65 2.21e-5 / 4.49e-3 / 3.30e-5 / 5.11e-2. The asymmetric B converges (7x from NS33 to NS65); its J and force through the fitted LASYM route barely improve, so they are route-limited (the pin has no native LASYM Cartesian field). LASYM physics is exercised at solver level with a genuine asymmetric exact reference. A native LASYM field route remains unavailable at the pin and is recorded as such.

### C2/C3 addendum, 2026-09-25: patched evaluator on solved states; sheared-A cold multigrid (office)

**Evaluator variant (pin + PR452 patch, tracked-diff SHA-256 `baf06693...`, clone `vmex-b5f5267-axisfix`; output `results/audit/measurement_c2_axisfix/`, separately labelled):** full measurements of the NS129 zero-TCON0 root (from its saved `spectral_state.npz`) and the NS257 root (from WOUT). B/J/force: NS129 near axis 6.69e-5 / 1.38e-2 / 2.03 (pinned 3.06e-4 / 9.88e-3 / 1.46); knots and cells 1.47e-4 / 1.39e-2 / 0.155; full midpoint 1.48e-5 / 2.40e-3 / 2.57e-2; near edge unchanged. NS257 near axis 1.10e-5 / 9.63e-3 / 1.42 (pinned 1.46e-4 / 9.56e-3 / 1.41); full midpoint 5.69e-7 / 1.56e-4 / 1.71e-3; edge unchanged. The patch removes most of the axis |B| error on the solved states but **not their near-axis current error**, which on the exact projection it reduced 440x. After the evaluator fix, the remaining near-axis J/force defect therefore belongs to the solved state's first cells (the solver's m=1 constraint and axis closure), not to evaluation. Two independent near-axis defects are now separated: evaluator axis row (fixed upstream in PR452) and solver axis cells (open).

**Sheared-A cold multigrid:** `BENCH_TCON0={default,0} run_vmex.py inputs/input.sheared_A_current --keep-terminal` (deck ladder NS 17/33/65, FTOL 1e-10/1e-12/1e-14; runs `sheared-a-multigrid-17-33-65-tcon0-{default,0}-20260925`). Both capped at 30000 NS65 iterations (IER 2): FSQ 2.2e-4 (default) and 5.6e-3 (zero). B/J/force 4.36e-2 / 0.196 / 0.222 and 4.44e-2 / 0.224 / 0.946; flux-label error 0.461 and 0.449. The standard cold path does not reach the sheared-A equilibrium at all. The accurate sheared-A states remain the exact projections; seeded solves drift (NS17, NS65) and cold solves fail.

**Next:** (1) C2 solver axis cells: compare the first two radial rows of the NS129 root with the exact projection mode by mode (m=0/1, R/Z/lambda) to locate the solver-side axis defect. (2) C3: stop spending iterations on sheared-A roots; record it as an unresolved solver limitation with a minimal reproducer (seed + NS65 deck + Newton trace), and move to C6 exterior-operator fixtures and C8 figures/README.

### C2 addendum, 2026-09-25: the solver-side near-axis current defect

**Mode-by-mode comparison (NS129 zero-TCON0 root vs exact projection, same internal representation):** geometry differences are at most 5.4e-7 of max|R| (R_cos) and 2.6e-6 (Z_sin), largest in rows j=1-3 (~2-5e-7) and 8e-10 at the edge. lambda (exact 0) peaks in the first rows: m=0, n=1 = 1.39e-4 at j=0/1 (the axis row copies j=1), 7.6e-6 at j=2, 6.1e-5 at j=3, 1.8e-6 at mid-radius.

**Lambda is not the cause:** with the patched evaluator, `axis_projection_ladder.py --state-npz <root> --zero-lambda-rows {none,4,129}` (runs `state-ladder-linear_extrapolation-20260925T10{4943,4959,5005}Z`) leaves near-axis J unchanged: s=1e-4 5.3e-3 / 4.9e-3 / 4.9e-3; s=1e-2 2.7e-2 / 2.8e-2 / 2.8e-2. The first two invocations passing the lambda option failed argument parsing (a zsh quoting error in my command) and are the extra run directories; the none case completed.

**Amplification estimate:** m=1 regularization divides by sqrt(s_1) = 0.088 at NS129, and J needs first derivatives across Delta s = 1/128, so ~1e-7 coefficient noise in rows 1-3 maps to O(1e-3) current error near the axis. This is consistent with the measured 5e-3 but not yet shown causally.
...
### Cross-code block, 2026-09-25: VMEX, VMEC2000, VMEC++ and DESC on the exact cases

**Upstream:** [uwplasma/vmex#452](https://github.com/uwplasma/vmex/pull/452) merged by the owner (`3b73d6f`). [uwplasma/vmex#453](https://github.com/uwplasma/vmex/pull/453) opened (refinement restart, rebased on main; the one failing office test pinned a single pass and now sets `_REFINE_RESTARTS = 0`). Not merged.

**VMEC-family matrix (local, one machine):** `benchmarks/cross_code_run.py` writes one deck per (case, NS) from `inputs/<case>_iota` (NS ladder 17/33/65/129/257 up to NS, FTOL 1e-12 intermediate / 1e-14 final, NITER 30000 per stage, TCON0 from the deck) and gives the same bytes to VMEX (`3b73d6f`), VMEC2000 (STELLOPT `3e1439d` binary) and VMEC++ 0.5.2. `benchmarks/score_wout_exact.py` scores every WOUT with the same code: flux label at points of each numerical surface, axis offset, B on native surfaces through one shared reader, iota, VMEC's own current (J = (currumnc e_u + currvmnc e_v)/sqrt(g), checked against the exact J), and VMEX's native field built from the same WOUT. `benchmarks/cross_code_summary.py` collects `results/cross_code/summary.json`.

Results: VMEX and VMEC2000 give identical scores on all 14 shared runs, including the capped integer 3-D (flux-label error 0.08-0.11, NS 33-129) and sheared-A (0.45-0.50) cold starts. VMEC++ matches on the axisymmetric and Solov'ev cases (small NS257 differences, e.g. axis offset 1.9e-7 vs 5.6e-7 m). It writes no WOUT on integer 3-D and sheared A ("did not converge") and crashes on the LASYM deck (`RuntimeError: bad_optional_access` after 0.2 s). Its WOUTs carry no current harmonics. From VMEC2000's NS129 axisymmetric WOUT, VMEX's native J is 10.2x, 32.5x, 70.2x and 2.6x more accurate than the WOUT current at s = 0.25, 0.5, 0.75 and 0.99, and 0.02-0.3x (worse) on the first three surfaces.

**DESC (office, CPU, `4f48720`):** `benchmarks/desc_axis_volume_check.py` on the saved integer 3-D states and new M=10/12 solves: volume J 1.46e-3, 1.49e-4, 1.64e-5, 1.88e-6 (M = 6, 8, 10, 12); axis-ring flux-label error 1.2e-5 down to 1.4e-9. The DESC axis offset is resolution, not a defect. `benchmarks/desc_general_case.py`: symmetric Solov'ev M8 B/J 1.4e-9 / 4.1e-8 (converged); asymmetric Solov'ev M10 1.3e-9 / 5.1e-8 (converged; M8 hit the cap); sheared A M8 2.5e-4 / 1.0e-3 and M10 1.9e-5 / 1.1e-4, both at the 150-iteration cap. The first M10/M12 ladder attempt failed on `tomllib` in Python 3.10 (`evidence.py` now imports it optionally; failure receipts kept).

**Figures:** `benchmarks/plot_highlights.py` -> `figures/highlight_{upstream_fixes,code_parity,field_routes,hard_cases}.png` with `figures/highlight_manifest.json`. Palette validated (4 slots pass; contrast warning covered by marker shapes and direct labels). All four were inspected and revised: tick overlap, a hidden series, and label collisions fixed; hard-case bars compare the same mid-radius quantity for every code.

**Next:** ask the owner before reporting the VMEC++ LASYM crash upstream; C6 NESTOR fixture; keep #453 under review.
