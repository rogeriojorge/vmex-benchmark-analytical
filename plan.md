...
| P2 | Partial | Native integer 3-D projected volume B/J/force passes first targets at NS=129 without solving; continuum gauge identity and finite NS33 surface projection are separately measured. Symmetric Solov'ev projection and LASYM WOUT round trip also run. Higher derivatives, LASYM volume and other file routes remain. |
| P3 | Partial | VMEX axisymmetric and integer-3D evidence remains as previously reported. Added matched held-out DESC axisymmetric and integer-3D base/remapped projections and fixed-iota solves at angular `M=N=6/8` (radial `L=8/10`), exact boundary-fit and sampled-Jacobian checks, and separate common-sample figures. DESC M6 base/remapped converged scores agree to about 0.1%; the remapped M8 run remains above the optimizer gtol after 120 iterations. This is early DESC-only diagnostic evidence: no independent transform/derivative certificate, VMEX TCON0 recovery ladder, VMEC2000/VMEC++ or GVEC result, so the P3 exit and any VMEX default recommendation remain open. |
...
| P10 | Partial | Reference, axisymmetric, integer 3-D projection/loose-root, short warm trajectory, force-localization, constraint-switch, gauge-scan, gauge-surface, and new VMEX/DESC coordinate-comparison figures generated from saved data and visually inspected; broader matched performance and resolution work remains. |
